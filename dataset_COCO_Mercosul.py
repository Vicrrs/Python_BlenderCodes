...
def rename_plate(letters: str):
    ...
    for i in letters:
        ...
        copy.location = (x, -0.035764, 0.0004) 
...
def random_letters(is_mercosul = True):
    ...
    letras_placa = ''
    if is_mercosul:
        letras_placa=letras_placa.join(np.random.choice(alfabeto, 3))
        letras_placa=letras_placa+ ''.join(np.random.choice(numbers, 1))
        letras_placa=letras_placa + ''.join(np.random.choice(alfabeto,1))
        letras_placa=letras_placa + ''.join(np.random.choice(numbers, 2))

    else:
        letras_placa=letras_placa.join(np.random.choice(alfabeto, 3))
        letras_placa=letras_placa.join(np.random.choice(alfabeto, 4))
...
if __name__=="__main__":
    for i in range(5):
        ...
        save_coco_annotations_to_json(f'/media/vicrrs/Novo volume/CILIA/placas/Placa_mercosul/images_plates/ann/{letras_placa}_coco.json', annotations)
        
        pos_camera = move_plate(plate)
        render_image(Path(f'/media/vicrrs/Novo volume/CILIA/placas/Placa_mercosul/images_plates/{letras_placa}'))
            
        path_txt = f'/media/vicrrs/Novo volume/CILIA/placas/Placa_mercosul/images_plates/ann/{letras_placa}.txt'
        save_image_points = True
        path_save_image_points = f"/media/vicrrs/Novo volume/CILIA/placas/Placa_mercosul/images_plates/img/{letras_placa}.png"
