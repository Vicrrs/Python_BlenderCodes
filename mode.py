...
def rename_plate(letters: str):
    ...
    objs_temp=[]
    letter_info = []  # Armazenar informações sobre as letras
    for idx, i in enumerate(letters):
        ...
        letter_info.append({
            'letter': i,
            'position': tuple(copy.location),
            'rotation': get_rotation_in_degrees(copy)
        })
    ...
    bpy.ops.object.select_all(action='DESELECT')
    return objs_temp, placa, letter_info



def get_letter_info(obj_plate):
    letter_info = []
    for child in obj_plate.children:
        letter_info.append({
            'letter': child.name,
            'position': tuple(child.location),
            'rotation': get_rotation_in_degrees(child)
        })
    return letter_info
...
def save_coco_annotations_to_json(filename, annotations, letter_info=None):
    data = {
        "annotations": [],
        "letters": []
    }

    # Adicionando informações das anotações
    ...
    # Adicionando informações das letras
    if letter_info:
        for info in letter_info:
            ...
            data["letters"].append(letter_entry)
...
def save_coco_annotations(filename, annotations, letter_info):
    with open(filename, 'w') as f:
        for ann in annotations:
            f.write(f"{ann[0]},{ann[1][0]},{ann[1][1]},{ann[1][2]},{ann[1][3]}\n")
        
        for info in letter_info:
            f.write(f"{info['letter']},{info['position'][0]},{info['position'][1]},{info['position'][2]},{info['rotation'][0]},{info['rotation'][1]},{info['rotation'][2]}\n")
...
if __name__=="__main__":
    for i in range(5):
        ...
        obj_all, plate, letter_info=rename_plate(letras_placa)
        annotations = []
        camera = bpy.data.objects['Camera']
