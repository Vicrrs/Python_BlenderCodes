...
def save_coco_annotations(filename, annotations):
    with open(filename, 'w') as f:
        for ann in annotations:
            f.write(f"{ann[0]},{ann[1][0]},{ann[1][1]},{ann[1][2]},{ann[1][3]},{ann[2][0]},{ann[2][1]},{ann[2][2]}\n")


def set_gray_scale_material(obj, gray_scale_value):
    """
    Define o material do objeto para uma escala de cinza específica.
    """
    # Cria um novo material com a cor em escala de cinza
    mat = bpy.data.materials.new(name="GrayScaleMaterial")
    mat.use_nodes = True
    bsdf = mat.node_tree.nodes["Principled BSDF"]
    bsdf.inputs['Base Color'].default_value = (gray_scale_value, gray_scale_value, gray_scale_value, 1)

    # Atribui o material ao objeto
    if obj.data.materials:
        obj.data.materials[0] = mat
    else:
        obj.data.materials.append(mat)

def apply_gray_scale_to_letters(objs):
    """
    Aplica uma escala de cinza única a cada letra/objeto.
    """
    for obj in objs:
        gray_scale_value = random.uniform(0, 1)  # Gera um valor aleatório entre 0 (preto) e 1 (branco)
        set_gray_scale_material(obj, gray_scale_value)


def save_original_materials(objs):
    # Salvando os materiais originais dos objetos
    original_materials = {obj: obj.data.materials[0] for obj in objs}
...
def restore_original_materials(objs, original_materials):
    ...
    for obj in objs:
        obj.data.materials[0] = original_materials[obj]

def render_plate_with_original_colors(objs, plate, letras_placa):
    # Renderizando a placa com as cores originais
    render_image(Path(f'/home/vicrrs/CILIA/placas/placa_antiga/images_plates/ORIGINAL_{letras_placa}'))

def render_plate_with_gray_scale(objs, plate, letras_placa):
    # Aplicando escala de cinza e renderizando a placa
    apply_gray_scale_to_letters(objs)
    render_image(Path(f'/home/vicrrs/CILIA/placas/placa_antiga/images_plates/GRAY_{letras_placa}'))

    
if __name__ == "__main__":
    for i in range(2):
        bpy.data.worlds["World"].node_tree.nodes["Background"].inputs[1].default_value = 0.5    
        delete_letters('PLACA')  # Certifique-se de que isso remove todas as letras e números corretamente
        set_point_light()
        ...
        # Salve as anotações COCO em um arquivo (descomente se necessário)
        # save_coco_annotations_to_json(f'/caminho/para/anotacoes/{letras_placa}_coco.json', annotations)
        ...
        render_plate_with_original_colors(obj_all, plate, letras_placa)

        # Renderiza a placa com as letras em escala de cinza
        render_plate_with_gray_scale(obj_all, plate, letras_placa)
