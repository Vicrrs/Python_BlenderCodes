...
def apply_gray_scale_to_letters(objs):
    ...
    """
    for obj in objs:
        letra = obj.name.split(".")[0]
        gray_scale_value = gray_scale_values[letra]
...
def set_plate_material(obj_plate):
    ...
    """
    mat = bpy.data.materials.get("auto.inst_material.10")
    if not mat:
...
def render_plate_with_original_colors(objs, plate, letras_placa):
    ...
    render_image(Path(f'C:\\Users\\rozas\\Documents\\CILIA\\img_plate\\images_plates_GRAY\\ORIGINAL_{letras_placa}'))
...
def render_plate_with_gray_scale(objs, plate, letras_placa):
    ...
    render_image(Path(f'C:\\Users\\rozas\\Documents\\CILIA\\img_plate\\images_plates_GRAY\\GRAY_{letras_placa}'))
...
if __name__ == "__main__":
    for i in range(3):
        ...
        # Aplica o material 'Material.003' na 'PLACA' para a imagem original
        set_plate_original_material(plate)  # Corrigido para corresponder ao nome da função definida

        # Renderiza a placa com as cores originais
        render_plate_with_original_colors(obj_all, plate, letras_placa)

        # Aplica o material 'auto.inst_material.0' na 'PLACA' para a imagem em escala de cinza
        set_plate_material(plate)
